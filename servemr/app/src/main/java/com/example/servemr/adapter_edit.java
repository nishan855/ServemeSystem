...
public class adapter_edit extends RecyclerView.Adapter<adapter_edit.holder> {
    ...
    public adapter_edit(Context ctx, List<model> tasklist ){
        this.ctx=ctx;
    ...
    @NonNull
    @Override
    public adapter_edit.holder onCreateViewHolder(@NonNull ViewGroup vg, int viewType) {
        View vw= LayoutInflater.from(ctx).inflate(R.layout.users_row_layout,vg,false);
        return new adapter_edit.holder(vw);
    }

    @Override
    public void onBindViewHolder(@NonNull adapter_edit.holder myholder, int pos) {
