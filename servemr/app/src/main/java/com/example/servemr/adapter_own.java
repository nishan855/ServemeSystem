...
public class adapter_own extends RecyclerView.Adapter<adapter_own.holder> {
    ...
    public adapter_own(Context ctx,List<model> tasklist ){
        this.ctx=ctx;
    ...
    @NonNull
    @Override
    public adapter_own.holder onCreateViewHolder(@NonNull ViewGroup vg, int viewType) {
        View vw= LayoutInflater.from(ctx).inflate(R.layout.users_row_layout,vg,false);
        return new adapter_own.holder(vw);
    }

    @Override
    public void onBindViewHolder(@NonNull adapter_own.holder myholder, int pos) {
        ...
        myholder.vbid.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Intent intent= new Intent(ctx,view_bid.class);
                intent.putExtra("ctxid",tk_id);
